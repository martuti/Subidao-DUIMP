...
#atributos = '\\ATRIBUTOS_POR_NCM_Teste.json'

path = "C:\\Users\\AMARTU01\\OneDrive - Donaldson Company, Inc\\Desktop\\DUIMP\\Plan DUIMP Project"
...
def salvar_respostas():
        try:
            ...
            def processar_resposta(resposta, atributos, nivel=0):
                ...
                elif forma_preenchimento == "NUMERO_INTEIRO":
                    try:
                        valor_resposta = int(valor_resposta)
                    except ValueError:
                        print(f"Valor inválido para NUMERO_INTEIRO: '{valor_resposta}' no atributo '{atributo_detalhes.get('codigo')}'")
                        valor_resposta = None

                elif forma_preenchimento == "NUMERO_REAL":
                    try:
                        valor_resposta = int(valor_resposta)
                    except ValueError:
                        print(f"Valor inválido para NUMERO_REAL: '{valor_resposta}' no atributo '{atributo_detalhes.get('codigo')}'")
                        valor_resposta = None

                elif forma_preenchimento == "TEXTO":
                    valor_resposta = valor_resposta.strip()

                else:
                    print(f"Tipo de dado desconhecido: '{forma_preenchimento}' para o atributo '{atributo_detalhes.get('codigo')}'")
                    valor_resposta = None
            ...
            try:
                ...
                for i, registro in enumerate(registros, start=primeira_linha_vazia):
                    ...
                    if (registro.get("Forma Preenchimento") == "NUMERO_INTEIRO" or registro.get("Forma Preenchimento") == "NUMERO_REAL") and isinstance(registro.get("Resposta"), int):
                ...
                try:
                    ...
                    for i, registro in enumerate(registros, start=2):  # Começa na linha 2
                        ...
                        forma_preenchimento = registro.get("Forma Preenchimento")

                        if (forma_preenchimento == "NUMERO_INTEIRO" or forma_preenchimento == "NUMERO_REAL") and isinstance(resposta, int):
                            cell = ws.cell(row=i, column=13)  # Coluna 13 para "Resposta"
                            cell.value = resposta
                            cell.number_format = '0'  # Formato numérico sem casas decimais
                        else:
                            # Preenche outras respostas na mesma coluna sem alterar o formato
                            ws.cell(row=i, column=13, value=resposta)

                    # Salva as alterações no arquivo
                    wb.save(planilha_path)
                    wb.close()

                    # Tenta executar o script após salvar a planilha
                    script_path = os.path.join(r"C:\\Users\\amartu01\\OneDrive - Donaldson Company, Inc\\Desktop\\DUIMP\\Robo DUIMP", "automacao_duimp_cherps.py")
                    try:
                        subprocess.run(["python", script_path], check=True)
                    except subprocess.CalledProcessError as e:
                        return jsonify({"error": f"Erro ao executar o script: {e}"}), 500

                    # Mensagem de sucesso após salvar e executar o script
